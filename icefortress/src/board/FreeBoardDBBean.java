...
public class FreeBoardDBBean {
    ...
    public ArrayList<FreeBoardDataBean> getList() { 
    	...
    	try {
    		conn = getConnection();
    		String SQL = "select * from freeBoard where commentID = 0 and available = 1 order by writtenID desc, replyID asc";
    ...
    public int delete(int writtenID, int replyID) {
        ...
        try {
        	...
        	pstmt = conn.prepareStatement("select * from freeBoard where writtenID = ? and replyID = ?"); // 그 글의 댓글도 같이 불러옴
        	pstmt.setInt(1, writtenID);
        	pstmt.setInt(2, replyID);
        	rs = pstmt.executeQuery();
        	while(rs.next()) {
