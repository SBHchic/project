...
$(document).ready(function(){
	...
	// freeBoard_writeForm.jsp 에서 글쓰기 버튼을 눌렀을 때
	...
	// freeBoard_view.jsp 에서 삭제버튼을 눌렀을 때
	$("#deleteWritten").click(function(){
		if(confirm("정말로 삭제하시겠습니까?") == true){
			...
			$.ajax({
				type:"post",
				url:"freeBoard_deleteWrittenPro.jsp",
				data:query,
				success:function(data){
					if (data == 1){
						location.href="freeBoard.jsp?pageNumber="+arr[2];
					} else if (data == 0){
						alert("접근 권한이 없습니다.");
					} else {
						alert("삭제에 실패했습니다.");
	...
	// freeBoard_updateWrittenForm.jsp 에서 수정버튼을 눌렀을 때
	$("#submit_update").click(function(){
		...
		if (status) {
			...
			$.ajax({
				type:"post",
				url:"freeBoard_updateWrittenPro.jsp",
				data:query,
				success:function(data){
					if (data == 1){
						location.href="freeBoard_view.jsp?writtenID="+arr[0]+"&replyID="+arr[1]+"&pageNumber="+arr[2];
					} else {
						alert("수정에 실패했습니다.");
					}
				}
	...
	// freeBoard_writeReplyForm.jsp 에서 답글쓰기 버튼을 눌렀을 때
	$("#submit_writeReply").click(function(){
		...
		if (status) {
			...
			$.ajax({
				type:"post",
				url:"freeBoard_writeReplyPro.jsp",
				data:query,
				success:function(data){
					if (data == 1){
						location.href="freeBoard.jsp?pageNumber="+arr[1];
					} else {
						alert("작성에 실패했습니다.");
					}
				}
	...
	// freeBoard_view.jsp에서 등록버튼을 눌렀을 때 (댓글)
	$("#writeComment").click(function(){
		...
		if (status) {
			...
			$.ajax({
				type:"post",
				url:"freeBoard_writeCommentPro.jsp",
				data:query,
				success:function(data){
					if (data == 1){
						location.reload();
...
function page(pageBtn){
	...
	location.href="freeBoard.jsp?pageNumber="+pageNum;
}
...
function deleteComment(deleteObject){
	if(confirm("정말로 삭제하시겠습니까?") == true){
		...
		$.ajax({
			type:"post",
			url:"freeBoard_deleteCommentPro.jsp",
			data:query,
			success:function(data){
				if (data == 1){
					location.reload();
...
function updateComment(updateComment){
	...
	if(!$("#content_"+tmp).val()){ // 수정할 내용을 입력하지 않았을 경우
		...
		$.ajax({
			type:"post",
			url:"freeBoard_updateCommentPro.jsp",
			data:query,
			success:function(data){
				if (data == 1){
					location.reload();
...
function writeCommentReply(commentReply){
	...
	if(!$("#contentRe_"+tmp).val()){ // 대댓글 내용을 입력하지 않았을 경우
		...
		$.ajax({
			type:"post",
			url:"freeBoard_writeCommentReplyPro.jsp",
			data:query,
			success:function(data){
				if (data == 1){
					location.reload();
